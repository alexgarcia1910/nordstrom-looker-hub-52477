...
export const CustomerDomainV8 = () => {
  ...
  return <div className="p-4 sm:p-6 lg:p-12">
      <div className="max-w-[1800px] mx-auto">
        ...
        <div className="mb-6 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
          ...
          <div>
            <h2 className="text-2xl font-semibold text-foreground mb-2">Customer</h2>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Select value={subdomainFilter} onValueChange={setSubdomainFilter}>
              ...
              <SelectContent className="bg-card z-50">
                ...
                {uniqueSubdomains.map(subdomain => <SelectItem key={subdomain} value={subdomain}>
                    {subdomain}
                  </SelectItem>)}
              </SelectContent>
            </Select>
            ...
            <Select value={ownerFilter} onValueChange={setOwnerFilter}>
              ...
              <SelectContent className="bg-card z-50">
                ...
                {uniqueOwners.map(owner => <SelectItem key={owner} value={owner}>
                    {owner}
                  </SelectItem>)}
              </SelectContent>
            </Select>
        ...
        <div className="space-y-4">
          {filteredData.length === 0 ? <div className="text-center py-12 text-muted-foreground">
              No results found. Try adjusting your filters.
            </div> : filteredData.map(item => <Card key={item.id} className="p-6 hover:shadow-md transition-all cursor-pointer group relative">
                <div className="flex flex-col sm:flex-row items-start justify-between gap-4">
                  {/* Left side - Icon and Content */}
                  <div className="flex items-start gap-4 flex-1 min-w-0">
                    {/* Icon */}
                    <div className="mt-1 flex-shrink-0">
                      {item.type === "Dashboard" ? <LayoutGrid className="h-5 w-5 text-foreground" /> : <Search className="h-5 w-5 text-foreground" />}
                    </div>
                  ...
                  <div className="flex items-start gap-3 flex-shrink-0">
                    <Badge variant="outline" className={cn("text-xs whitespace-nowrap", getStatusBadgeColor(item.status))}>
                  ...
                  toggleFavorite(item.id);
                        ...
                        <Heart className={cn("h-4 w-4", favorites.has(item.id) ? "fill-red-500 text-red-500" : "text-muted-foreground")} />
                      </button>
